...
local CLI. Nothing is deployed yet.
...
main site, so the public address is **`robertsamples.com/antismashtableconverter/`**. A
...
route `robertsamples.com/antismashtableconverter*`, or:
...
1. **Add the `Disallow` to the main site's robots.txt.** Only a robots.txt at a domain root is
   ...
   proxied path. `robertsamples.com/robots.txt` needs its own line:
...
2. **Link to it from the main site's navigation.** Internal links are the primary discovery
   path, and are the reason the subdirectory layout is worth the Worker.

3. **Add `https://robertsamples.com/antismashtableconverter/` to the main sitemap**, and keep
